...
def _wrap_unique_genrule_impl(ctx):
    # Symlink all the outputs from the genrule into the same location
    # of the current build configuration. NOTE: this will not work if
    # the dependent was already in the "unique" configuration!!
    src_outs = ctx.files.genrule_target
    dst_files = []
    for src_file, dst in zip(src_outs, ctx.attr.genrule_outs):
