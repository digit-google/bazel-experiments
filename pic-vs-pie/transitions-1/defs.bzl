...
def _modify_copy_and_linkopt_settings(settings, remove_copts, add_copt,
                                      remove_linkopts, add_linkopt):
    ...
    Args:
      ...
      remove_copts: A list of compiler options to remove from
          settings['//command_line_options:copt']
      add_copt: A new compiler option to enforce in the result.
      remove_linkopts: A list of linker options to remove from
          settings['//command_line_option:linkopt']
      add_linkopt: A new linker option to enforce in the result.
...
def _to_elf_shared_library_transition_impl(settings, attr):
    return _modify_copy_and_linkopt_settings(
        settings,
        ["-fPIE", "-fpie", "-fpic"],
        "-fPIC",
        ["-fpie", "-pie"],
