...
def _wrap_unique_genrule_impl(ctx):
    # If the target genrule was in a different build configuration, symlink
    # all its outputs at the same relative output location for the dependent
    # build configuration. Otherwise, do not do anything but just pass the
    # output paths in the DefaultInfo provider.
    target = ctx.attr.genrule_target[0]
    dst_files = []

    # The destination path for ouputs of the current package.
    dst_dir = "%s/%s" % (ctx.bin_dir.path, ctx.label.package)

    for src_file, dst in zip(target.files.to_list(), ctx.attr.genrule_outs):
        # Pre-compute the path of ctx.actions.declare_file(dst) without
        # actually calling the function.
        dst_path = "%s/%s" % (dst_dir, dst)
        if dst_path == src_file.path:
            # For the case where the current build configuration is the
            # same as the 'unique' one used for the genrule() target.
            dst_files.append(src_file)
        else:
            # Otherwise, just creates some symlinks.
            dst_file = ctx.actions.declare_file(dst)
            if dst_file.path != dst_path:
                fail("Unexpected destination path: %s, expected %s" % (dst_file.path, dst_path))
            dst_files.append(dst_file)
            ctx.actions.symlink(output = dst_file, target_file = src_file)
    return [DefaultInfo(files = depset(dst_files))]
...
_wrap_unique_genrule = rule(
    ...
    attrs = {
        ...
        "genrule_outs": attr.string_list(
            mandatory = True,
            doc = "List of output filenames",
        ),
        "_allowlist_function_transition": attr.label(
            default = "@bazel_tools//tools/allowlists/function_transition_allowlist",
        ),
